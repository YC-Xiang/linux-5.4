...
static bool nowayout = WATCHDOG_NOWAYOUT; /// default 0 CONFIG_WATCHDOG_NOWAYOUT意思是打开watchdog后不会停止，close dev/watchdog文件也没用
static int ictype;
...
static void rtsx_wdt_ping(void)
{
	...
	spin_lock_irqsave(&rtsx_wdt_device.io_lock, flag);
	rts_set_field(wdt_reg + WATCHDOG_CTL, 1, 1, 0); /// enable watchdog interrupt?
	spin_unlock_irqrestore(&rtsx_wdt_device.io_lock, flag);
}
...
static int rtsx_wdt_probe(struct platform_device *pdev)
{
	...
	rtsx_wdt_stop(); /// disable watchdog

	if (rtsx_wdt_set(timeout)) /// timeout: 8s
		rtsx_wdt_set(WATCHDOG_TIMEOUT);

	ret = register_restart_handler(&wdt_restart_handler); /// 注册进全局的restart_handler_list,在系统reboot会调用wdt_restart_handler.
	if (ret) {
