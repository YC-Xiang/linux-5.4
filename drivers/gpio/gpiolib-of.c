		     ...
		     const char *propname, int index, enum of_gpio_flags *flags)
{
	...
	ret = of_parse_phandle_with_args_map(np, propname, "gpio", index, /// proname == xxx-gpios
					     &gpiospec);
	if (ret) {
			       ...
			       unsigned int idx, unsigned long *flags)
{
	...
	for (i = 0; i < ARRAY_SIZE(gpio_suffixes); i++) {
		if (con_id)
			snprintf(prop_name, sizeof(prop_name), "%s-%s", con_id, /// xxx-gpios or xxx-gpio
				 gpio_suffixes[i]);
		else
			snprintf(prop_name, sizeof(prop_name), "%s", /// gpios or gpio
				 gpio_suffixes[i]);
