		...
		void (*release)(struct device *dev))
{
	...
	dma_set_coherent_mask(&gadget->dev, DMA_BIT_MASK(32));
...
static int udc_bind_to_driver(struct usb_udc *udc, struct usb_gadget_driver *driver)
{
	...
	ret = driver->bind(udc->gadget, driver); /// composite.c 的.bind composite_bind
	if (ret)
		goto err1;
	ret = usb_gadget_udc_start(udc); /// 调用到 udc driver core.c的.udc_start 启动udc
	if (ret) {
...
int usb_gadget_probe_driver(struct usb_gadget_driver *driver)
{
	...
	mutex_lock(&udc_lock);
	if (driver->udc_name) { /// 没有特别指定udc的name，跳过
		list_for_each_entry(udc, &udc_list, list) {
		...
		list_for_each_entry(udc, &udc_list, list) {
			...
			if (!udc->driver)
				goto found; /// this way 我们取到了底层的udc
		}
	}
	...
	return ret;
found:
	ret = udc_bind_to_driver(udc, driver); /// 将底层udc和上层gadget driver绑定起来
	mutex_unlock(&udc_lock);
	return ret;
}
