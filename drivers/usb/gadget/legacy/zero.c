...
static struct usb_device_descriptor device_desc = {
	.bLength =		sizeof device_desc, /// 设备描述符长度
	.bDescriptorType =	USB_DT_DEVICE, /// 描述符类型，这里是设备描述符

	/* .bcdUSB = DYNAMIC */ /// usb spec release number
	.bDeviceClass =		USB_CLASS_VENDOR_SPEC,

	.idVendor =		cpu_to_le16(DRIVER_VENDOR_NUM),
	.idProduct =		cpu_to_le16(DRIVER_PRODUCT_NUM),
	.bNumConfigurations =	2, /// 可能有的配置数量，这里正好是loopback和source link
};
...
#define USB_GZERO_SS_DESC	(USB_GADGET_FIRST_AVAIL_IDX + 0) /// 3
#define USB_GZERO_LB_DESC	(USB_GADGET_FIRST_AVAIL_IDX + 1) /// 4
...
static struct usb_configuration loopback_driver = {
	.label          = "loopback",
	.bConfigurationValue = 2, /// 当使用SetConfiguration请求时所指定的配置索引值。
...
static int zero_bind(struct usb_composite_dev *cdev)
{
	...
	func_inst_lb = usb_get_function_instance("Loopback"); /// 根据字符串找到f_loopback.c中的usb_function_driver，调用.alloc_inst即loopback_alloc_instance
	...
	lb_opts->bulk_buflen = gzero_options.bulk_buflen; /// 4096
	lb_opts->qlen = gzero_options.qlen; /// 32

	func_lb = usb_get_function(func_inst_lb); /// 调用f_loopback.c中loopback_alloc
	if (IS_ERR(func_lb)) {
...
static struct usb_composite_driver zero_driver = {
	...
	.dev		= &device_desc,
	.strings	= dev_strings,
	.max_speed	= USB_SPEED_SUPER, /// usb 3.0 速度
	.bind		= zero_bind,
	.unbind		= zero_unbind,
