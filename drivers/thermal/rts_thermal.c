...
u32 temp2code(struct rts_thermal_zone *zone, u32 actk)
{
	...
	x = actk; /// e.g 7500 + 27315 = 34815. 7500 from dts
	x1 = actual_k[i]; /// 34815
	y1 = zone->thermal_k[i]; /// 34815 from dts
	x2 = actual_k[i+1]; /// 35315
	y2 = zone->thermal_k[i+1]; /// 35315 from dts
	thk = (x-x1)*(y2-y1)/(x2-x1) + y1; /// thk = 34815

	thk = (thk - 27315) / 100 * 1024; /// thk = 75 * 1024 = 76800
