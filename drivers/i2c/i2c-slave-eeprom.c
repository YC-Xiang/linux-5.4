...
static int i2c_slave_eeprom_slave_cb(struct i2c_client *client, /// 调用i2c_slave_event会到这儿
				     enum i2c_slave_event event, u8 *val)
{
	...
	case I2C_SLAVE_READ_PROCESSED:
		...
		eeprom->buffer_idx++;
		fallthrough; /// I2C_SLAVE_READ_PROCESSED会fall through
...
static int i2c_slave_eeprom_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct eeprom_data *eeprom;
	int ret;
	unsigned int size = FIELD_GET(I2C_SLAVE_BYTELEN, id->driver_data) + 1; /// I2C_SLAVE_DEVICE_MAGIC(2048 / 8,  0) 256 bytes
	...
	eeprom->num_address_bytes = flag_addr16 ? 2 : 1; /// 1
	eeprom->address_mask = size - 1;
	eeprom->read_only = FIELD_GET(I2C_SLAVE_FLAG_RO, id->driver_data); /// 0
