...
static inline u32 dw_readl_pad(struct dw_spi *dws, u32 offset)
{
	return __raw_readl(dws->reg_pad + offset);
}
...
static inline void dw_writel_pad(struct dw_spi *dws, u32 offset, u32 val)
{
	__raw_writel(val, dws->reg_pad + offset);
}
...
static inline void spi_mask_intr(struct dw_spi *dws, u32 mask)
{
	...
	dw_writel(dws, DW_SPI_IMR, new_mask); /// 把中断mask都写成0，disable irq
}
...
{
	spi_enable_chip(dws, 0); /// disable ssi
	spi_mask_intr(dws, 0xff); /// disable irq
	dw_writel(dws, SSI_IRQ_ENABLE, DONE_INT_DIS); /// disable transfer done interrupt
