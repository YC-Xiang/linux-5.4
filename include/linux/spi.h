...
struct spi_device {
	...
	u8			bits_per_word; /// 每个基本的SPI传输涉及多少位。寄存器里多少位会被发送出去？使用bits_per_word来表示
	bool			rt;
	u32			mode;
...
#define	SPI_LOOP	0x20			/* loopback mode */ ///回环模式，就是SO、SI连接在一起
#define	SPI_NO_CS	0x40			/* 1 dev/bus, no chipselect */ /// 只有一个SPI设备，没有片选信号，也不需要片选信号
...
#define	SPI_3WIRE_HIZ	0x8000			/* high impedance turnaround */
	...
	int			cs_gpio;	/* LEGACY: chip select gpio */ /// 这是可选项，也可以把spi_device的片选引脚记录在这里
...
struct spi_controller {
	...
	struct list_head list; /// 链接到全局的 spi_controller list
	 ...
	 */
	s16			bus_num; /// spi bus 的编号，比如某 SoC有3个 SPI 控制，那么这个结构描述的是第几个
...
#define SPI_MASTER_GPIO_SS		BIT(5)	/* GPIO CS must select slave */
	 ...
	 */
	int			(*setup)(struct spi_device *spi); /// 主要设置SPI控制器和工作方式、clock等
	 ...
	 */
    /// 添加消息到队列的方法。这个函数不可睡眠。它的职责是安排发生的传送并且调用注册的回调函 complete()。这个不同的控制器要具体实现，传输数据最后都要调用这个函数
	...
	int (*transfer_one)(struct spi_controller *ctlr, struct spi_device *spi, /// 新方法，代替transfer函数
