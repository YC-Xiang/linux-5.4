...
void image_print_contents(const void *ptr)
{
	...
	printf("%sImage Type:   ", p); /// ARM Linux Kernel Image (uncompressed)
	image_print_type(hdr);
	printf("%sData Size:    ", p); /// 2215408 Bytes = 2.1 MiB
	genimg_print_size(image_get_data_size(hdr));
	printf("%sLoad Address: %08x\n", p, image_get_load(hdr)); /// Load Address: 81000000
	printf("%sEntry Point:  %08x\n", p, image_get_ep(hdr)); /// Entry Point:  81000000
...
static void print_decomp_msg(int comp_type, int type, bool is_xip)
{
	...
	if (comp_type == IH_COMP_NONE)
		printf("   %s %s\n", is_xip ? "XIP" : "Loading", name); /// this way "Loading Kernel Image"
	else
...
#ifndef USE_HOSTCC
	flush_cache(load, ALIGN(image_len, ARCH_DMA_MINALIGN));
#endif
	puts("OK\n");
			     ...
			     const char **fit_uname_kernel)
{
	...
	if (!img_addr) { /// 如果直接输入bootm指令 则img_addr=NULL
		kernel_addr = image_load_addr;
		debug("*  kernel: default image load address = 0x%08lx\n",
		      image_load_addr); /// 会从上面定义的image_load_addr=CONFIG_SYS_LOAD_ADDR(rlxboard.h) 0x80400000获取kernel，会失败。
		...
		kernel_addr = simple_strtoul(img_addr, NULL, 16); /// bootm {kernel_offset}(0xc0000)传入kernel_addr
...
ulong genimg_get_image(ulong img_addr)
{
	...
	if (img_addr < 0x80000000) {
		...
		get_data_length_from_nand(img_addr & 0x1fffffff,
			&data_len);
	...
	if (1) {
		...
		// spi_flash_read_external(img_addr, h_size, buf);
