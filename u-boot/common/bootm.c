			 ...
			 char * const argv[])
{
	...
	} else if (images.legacy_hdr_valid) { /// yes
...
static int bootm_load_os(bootm_headers_t *images, int boot_progress)
{
	...
	ulong image_start = os.image_start; /// 不包括kernel image头部信息
		    ...
		    int states, bootm_headers_t *images, int boot_progress)
{
	...
	if (!ret && (states & BOOTM_STATE_FINDOS))
		ret = bootm_find_os(cmdtp, flag, argc, argv); /// 填充bootm_headers_t images

	if (!ret && (states & BOOTM_STATE_FINDOTHER))
		ret = bootm_find_other(cmdtp, flag, argc, argv); /// 好像return 0了，没什么操作
...
static image_header_t *image_get_kernel(ulong img_addr, int verify)
{
	...
	if (verify) { /// verify == -1，so don't skip. bootm_start->images.verify = env_get_yesno("verify");
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		printf("## Booting kernel from Legacy Image at %08lx ...\n",
		       img_addr);
		hdr = image_get_kernel(img_addr, images->verify); /// hdr 就是linux的头部信息
		if (!hdr)
			return NULL;
		...
		case IH_TYPE_KERNEL: /// this case
		case IH_TYPE_KERNEL_NOLOAD:
			*os_data = image_get_data(hdr); /// 跳过hdr(kernel image起始地址)头部信息，跳到数据部分
			*os_len = image_get_data_size(hdr); /// 读取hdr(kernel image起始地址)头部信息中的kernel size
			break;
		case IH_TYPE_MULTI:
