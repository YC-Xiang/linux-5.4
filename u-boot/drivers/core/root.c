...
int dm_init(bool of_live)
{
	...
	if (CONFIG_IS_ENABLED(OF_PLATDATA_INST)) {
		...
		INIT_LIST_HEAD(DM_UCLASS_ROOT_NON_CONST); /// 走这边，初始化uclass链表
	}
	...
	if (CONFIG_IS_ENABLED(OF_PLATDATA_INST)) {
        ...
        /// 这里我们暂时只需要了解root根设备的udevice以及对应的uclass都已经创建完成。
		ret = device_bind_by_name(NULL, false, &root_info,
					  &DM_ROOT_NON_CONST); /// DM_ROOT_NON_CONST就是gd->dm_root
		if (ret)
			return ret;
		if (CONFIG_IS_ENABLED(OF_CONTROL))
			dev_set_ofnode(DM_ROOT_NON_CONST, ofnode_root());
		ret = device_probe(DM_ROOT_NON_CONST); /// 对根设备进行probe
		if (ret)
			return ret;
	}
...
static int dm_scan_fdt_node(struct udevice *parent, ofnode parent_node,
			    bool pre_reloc_only) /// parent:根设备 parent_node:根节点
{
	int ret = 0, err = 0;
	ofnode node;
	...
	for (node = ofnode_first_subnode(parent_node); ///// 获得blob设备树的offset偏移下的节点的第一个子节点
	     ofnode_valid(node);
	     node = ofnode_next_subnode(node)) { /// 下一个节点
		const char *node_name = ofnode_get_name(node);

		if (!ofnode_is_enabled(node)) { /// 判断节点状态是否是disable，如果是的话直接忽略
			pr_debug("   - ignoring disabled device\n");
			continue;
		}
		err = lists_bind_fdt(parent, node, NULL, NULL, pre_reloc_only); /// 解析绑定这个节点，dm_scan_fdt的核心
		if (err && !ret) {
			ret = err;
			debug("%s: ret=%d\n", node_name, ret);
		}
...
int dm_init_and_scan(bool pre_reloc_only)
{
	...
	ret = dm_init(CONFIG_IS_ENABLED(OF_LIVE)); /// DM初始化
	if (ret) {
