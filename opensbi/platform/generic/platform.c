				...
				unsigned long arg4)
{
	...
	fw_platform_lookup_special(fdt, root_offset); /// 匹配特殊platform的，可以跳过

	model = fdt_getprop(fdt, root_offset, "model", &len); /// 从设备树获取model. model=rtk,sheipa
	if (model)
		sbi_strncpy(platform.name, model, sizeof(platform.name));

	if (generic_plat && generic_plat->features) /// 跳过
	...
	fdt_for_each_subnode(cpu_offset, fdt, cpus_offset) {
		...
		generic_hart_index2id[hart_count++] = hartid; /// hart_count是有多少个cpu核心，hartid是每个核心有多少线程
	}
